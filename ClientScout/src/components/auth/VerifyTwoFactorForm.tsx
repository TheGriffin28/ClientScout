...
export default function VerifyTwoFactorForm() {
  ...
  useEffect(() => {
    const state = location.state as PendingTwoFactor | null;
    if (state?.userId && state.email) {
      setUserId(state.userId);
      setEmail(state.email);
      setRememberMe(Boolean(state.rememberMe));
      return;
    }

    const stored = localStorage.getItem("pendingTwoFactor");
    if (stored) {
      try {
        const parsed = JSON.parse(stored) as PendingTwoFactor;
        ...
        <div>
          <div className="mb-5 sm:mb-8">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">
              {hasSession
                ? `Enter the OTP sent to ${email} to complete your sign-in.`
